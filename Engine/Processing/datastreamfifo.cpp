    ...
    maxReadLength(maxReadLength_)
{
    ...
    qDebug() << "[DEBUG DataStreamFifo] Constructor started";
    qDebug() << "[DEBUG DataStreamFifo] Parameters - bufferSize:" << bufferSize_;
    qDebug() << "[DEBUG DataStreamFifo] Parameters - maxReadLength:" << maxReadLength_;
    
    try {
        int bufferSizeWithExtra = bufferSize + maxReadLength;
        qDebug() << "[DEBUG DataStreamFifo] Buffer size with extra:" << bufferSizeWithExtra;
        
        memoryNeededGB = sizeof(uint16_t) * bufferSizeWithExtra / (1024.0 * 1024.0 * 1024.0);
        qDebug() << "[DEBUG DataStreamFifo] Memory needed (GB):" << memoryNeededGB;
        
        std::cout << "DataStreamFifo: Allocating " << 2 * bufferSizeWithExtra / 1.0e6 << " MBytes for FIFO buffer." << std::endl;
        buffer = nullptr;

        qDebug() << "[DEBUG DataStreamFifo] Attempting to allocate FIFO buffer...";
        memoryAllocated = true;
        try {
            buffer = new uint16_t [bufferSizeWithExtra];
            qDebug() << "[DEBUG DataStreamFifo] FIFO buffer allocated successfully at:" << (void*)buffer;
        } catch (std::bad_alloc&) {
            qDebug() << "[DEBUG DataStreamFifo] ERROR: Failed to allocate FIFO buffer!";
            memoryAllocated = false;
            std::cerr << "Error: DataStreamFifo constructor could not allocate " << memoryNeededGB << " GB of memory." << std::endl;
        }

        if (!buffer) {
            qDebug() << "[DEBUG DataStreamFifo] ERROR: Buffer is null after allocation!";
            std::cerr << "Error: DataStreamFifo constructor could not allocate " << 2 * bufferSizeWithExtra << " bytes of memory." << std::endl;
        }
        
        qDebug() << "[DEBUG DataStreamFifo] Calling resetBuffer()...";
        resetBuffer();
        qDebug() << "[DEBUG DataStreamFifo] resetBuffer() completed";
    } catch (const std::exception& e) {
        qDebug() << "[DEBUG DataStreamFifo] EXCEPTION in constructor:" << e.what();
        throw;
    } catch (...) {
        qDebug() << "[DEBUG DataStreamFifo] UNKNOWN EXCEPTION in constructor!";
        throw;
    }
    
    qDebug() << "[DEBUG DataStreamFifo] Constructor completed successfully";
    qDebug() << "[DEBUG DataStreamFifo] === Constructor Exit ===";
}
