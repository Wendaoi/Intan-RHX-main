    ...
    usbBufferIndex(0)
{
    ...
    qDebug() << "[DEBUG USBDataThread] Constructor started";
    qDebug() << "[DEBUG USBDataThread] Parameters - controller:" << (void*)controller_;
    qDebug() << "[DEBUG USBDataThread] Parameters - usbFifo:" << (void*)usbFifo_;
    qDebug() << "[DEBUG USBDataThread] Parameters - parent:" << (void*)parent;
    
    try {
        qDebug() << "[DEBUG USBDataThread] Getting controller type...";
        ControllerType controllerType = controller->getType();
        qDebug() << "[DEBUG USBDataThread] Controller type:" << (int)controllerType;
        
        qDebug() << "[DEBUG USBDataThread] Getting max data streams...";
        int maxDataStreams = controller->maxNumDataStreams();
        qDebug() << "[DEBUG USBDataThread] Max data streams:" << maxDataStreams;
        
        qDebug() << "[DEBUG USBDataThread] Calculating buffer size...";
        int dataBlockSizeInWords = RHXDataBlock::dataBlockSizeInWords(controllerType, maxDataStreams);
        qDebug() << "[DEBUG USBDataThread] Data block size in words:" << dataBlockSizeInWords;
        
        bufferSize = (BufferSizeInBlocks + 1) * BytesPerWord * dataBlockSizeInWords;
        qDebug() << "[DEBUG USBDataThread] Calculated buffer size:" << bufferSize;
        
        memoryNeededGB = sizeof(uint8_t) * bufferSize / (1024.0 * 1024.0 * 1024.0);
        qDebug() << "[DEBUG USBDataThread] Memory needed (GB):" << memoryNeededGB;
        
        std::cout << "USBDataThread: Allocating " << bufferSize / 1.0e6 << " MBytes for USB buffer." << std::endl;
        usbBuffer = nullptr;

        qDebug() << "[DEBUG USBDataThread] Attempting to allocate USB buffer...";
        memoryAllocated = true;
        try {
            usbBuffer = new uint8_t [bufferSize];
            qDebug() << "[DEBUG USBDataThread] USB buffer allocated successfully at:" << (void*)usbBuffer;
        } catch (std::bad_alloc&) {
            qDebug() << "[DEBUG USBDataThread] ERROR: Failed to allocate USB buffer!";
            memoryAllocated = false;
            std::cerr << "Error: USBDataThread constructor could not allocate " << memoryNeededGB << " GB of memory." << std::endl;
        }
    } catch (const std::exception& e) {
        qDebug() << "[DEBUG USBDataThread] EXCEPTION in constructor:" << e.what();
        throw;
    } catch (...) {
        qDebug() << "[DEBUG USBDataThread] UNKNOWN EXCEPTION in constructor!";
        throw;
    }

//    cout << "Ideal thread count: " << QThread::idealThreadCount() << EndOfLine;
    
    qDebug() << "[DEBUG USBDataThread] Constructor completed successfully";
    qDebug() << "[DEBUG USBDataThread] === Constructor Exit ===";
}
