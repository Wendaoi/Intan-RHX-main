...
BoardIdentifier::BoardIdentifier(QWidget *parent_) :
    parent(parent_)
{
    qDebug() << "[DEBUG BoardIdentifier] === Constructor Entry ===\n";
    qDebug() << "[DEBUG BoardIdentifier] Constructor started";
    qDebug() << "[DEBUG BoardIdentifier] Parent widget pointer:" << (void*)parent_;
    qDebug() << "---- Intan Technologies ----\n";
    
    try {
        qDebug() << "[DEBUG BoardIdentifier] Checking FrontPanel DLL...";
        if (!okFrontPanel_TryLoadLib()) {
            qDebug() << "[ERROR BoardIdentifier] FrontPanel library load failed";
#ifdef _WIN32
            ...
            qDebug() << "[ERROR BoardIdentifier] FrontPanel DLL could not be loaded. Make sure this DLL is in the application start directory.";
#elif __APPLE__
            QMessageBox::warning(nullptr, "FrontPanel DyLib could not be loaded.", "FrontPanel DyLib could not be loaded. Make sure 'libokFrontPanel.dylib' is in the Frameworks directory\n"
                                                                                 "of the application");
            qDebug() << "[ERROR BoardIdentifier] FrontPanel DyLib could not be loaded. Make sure DyLib can be found.";
#elif __linux__
            ...
            qDebug() << "[ERROR BoardIdentifier] FrontPanel Shared Object could not be loaded. Make sure the .so is in the application start directory";
#endif
            return;
        }
        qDebug() << "[DEBUG BoardIdentifier] FrontPanel DLL loaded successfully";
        qDebug() << "[DEBUG BoardIdentifier] FrontPanel DLL loaded. Version:" << okFrontPanel_GetAPIVersionString();
        
        qDebug() << "[DEBUG BoardIdentifier] Creating okCFrontPanel device...";
        dev = new okCFrontPanel;
        qDebug() << "[DEBUG BoardIdentifier] okCFrontPanel device created at:" << (void*)dev;
        qDebug() << "[DEBUG BoardIdentifier] Constructor completed successfully";
        
    } catch (const std::exception& e) {
        qDebug() << "[ERROR BoardIdentifier] Standard exception in constructor:" << e.what();
        throw;
    } catch (...) {
        qDebug() << "[ERROR BoardIdentifier] Unknown exception in constructor";
        throw;
    }
}
...
{
    qDebug() << "[DEBUG getConnectedControllersInfo] === Function Entry ===\n";
    
    try {
        int i, nDevices;
        qDebug() << "[DEBUG getConnectedControllersInfo] Scanning USB for Opal Kelly devices...";
        nDevices = dev->GetDeviceCount();
        qDebug() << "[DEBUG getConnectedControllersInfo] Found" << nDevices << "Opal Kelly" << ((nDevices == 1) ? "device" : "devices") << "connected.";
        
        if (nDevices == 0) {
            qDebug() << "[DEBUG getConnectedControllersInfo] No devices found, returning empty list";
            return controllers;
        }

        for (i = 0; i < nDevices; ++i) {
            qDebug() << "[DEBUG getConnectedControllersInfo] Processing device" << i + 1 << "of" << nDevices;
            
            // Create Controller data structure
            ControllerInfo *controller = new ControllerInfo;
            qDebug() << "[DEBUG getConnectedControllersInfo] Created ControllerInfo for device" << i;
            ...
            qDebug() << "[DEBUG getConnectedControllersInfo] About to identify controller" << i;
            identifyController(controller, i);
            qDebug() << "[DEBUG getConnectedControllersInfo] Controller" << i << "identified successfully";

            // Add this Controller to the 'controllers' QVector.
            controllers.append(controller);
            qDebug() << "[DEBUG getConnectedControllersInfo] Controller" << i << "added to list";
        }
        
        qDebug() << "[DEBUG getConnectedControllersInfo] About to delete device object...";
        delete dev;
        qDebug() << "[DEBUG getConnectedControllersInfo] Device object deleted";
        
        qDebug() << "[DEBUG getConnectedControllersInfo] Function completed successfully with" << controllers.size() << "controllers";
        return controllers;
        
    } catch (const std::exception& e) {
        qDebug() << "[ERROR getConnectedControllersInfo] Standard exception:" << e.what();
        throw;
    } catch (...) {
        qDebug() << "[ERROR getConnectedControllersInfo] Unknown exception caught";
        throw;
    }
}
...
{
    qDebug() << "[DEBUG identifyController] Starting device identification for index:" << index;
    
    // Populate serialNumber field.
    qDebug() << "[DEBUG identifyController] Getting device serial number...";
    controller->serialNumber = dev->GetDeviceListSerial(index).c_str();
    qDebug() << "[DEBUG identifyController] Serial number:" << controller->serialNumber;

    // Populate usbVersion field.
    qDebug() << "[DEBUG identifyController] Getting device model...";
    QString modelName = opalKellyModelName(dev->GetDeviceListModel(index));
    qDebug() << "[DEBUG identifyController] Model name:" << modelName;
    if (modelName == "XEM6010LX45") {
        controller->usbVersion = USB2;
        qDebug() << "[DEBUG identifyController] USB version: USB2";
    } else if (modelName == "XEM6310LX45") {
        controller->usbVersion = USB3;
        qDebug() << "[DEBUG identifyController] USB version: USB3";
    } else if (modelName == "XEM7310A75") {
        controller->usbVersion = USB3_7310;
        qDebug() << "[DEBUG identifyController] USB version: USB3_7310";
    } else {
        controller->usbVersion = USB3_7310;
        qDebug() << "[DEBUG identifyController] Opal Kelly model name not recognized, assuming XEM7310";
        qDebug() << "[DEBUG identifyController] USB version: USB3_7310 (assumed)";
    }
    ...
    qDebug() << "[DEBUG identifyController] Initializing controller parameters...";
    controller->expConnected = false;
    controller->numSPIPorts = 0;
    controller->boardMode = UnknownUSB2Device;

    // Open device.
    qDebug() << "[DEBUG identifyController] Opening device by serial...";
    if (dev->OpenBySerial(dev->GetDeviceListSerial(index).c_str()) != okCFrontPanel::NoError) {
        qDebug() << "[ERROR identifyController] Device could not be opened. Is one connected?";
        return;
    }
    qDebug() << "[DEBUG identifyController] Device opened successfully";

    // Set up default PLL.
    qDebug() << "[DEBUG identifyController] Loading default PLL configuration...";
    dev->LoadDefaultPLLConfiguration();
    qDebug() << "[DEBUG identifyController] PLL configuration loaded";
    ...
    qDebug() << "[DEBUG identifyController] Determining bitfile...";
    QString bitfilename;
    if (controller->usbVersion == USB2) {
        bitfilename = ConfigFileXEM6010Tester;
        qDebug() << "[DEBUG identifyController] Selected bitfile for USB2:" << bitfilename;
    } else if (controller->usbVersion == USB3) {
        bitfilename = ConfigFileRHDController;
        qDebug() << "[DEBUG identifyController] Selected bitfile for USB3:" << bitfilename;
    } else {
        bitfilename = ConfigFileRHDController_7310;
        qDebug() << "[DEBUG identifyController] Selected bitfile for USB3_7310:" << bitfilename;
        //bitfilename = ConfigFileRHSController_7310;
    }

    // Upload bit file.
    qDebug() << "[DEBUG identifyController] Starting bitfile upload...";
    QString fullBitfilePath = QCoreApplication::applicationDirPath() + "/" + bitfilename;
    qDebug() << "[DEBUG identifyController] Full bitfile path:" << fullBitfilePath;
    
    if (!uploadFpgaBitfileQMessageBox(fullBitfilePath)) {
        qDebug() << "[ERROR identifyController] Bitfile upload failed";
    ...
    qDebug() << "[DEBUG identifyController] Bitfile uploaded successfully";
    
    qDebug() << "[DEBUG identifyController] Resetting board...";
    RHXController::resetBoard(dev);
    qDebug() << "[DEBUG identifyController] Board reset completed";

    // Read mode from board.
    qDebug() << "[DEBUG identifyController] Reading board mode...";
    int boardMode = RHXController::getBoardMode(dev);
    qDebug() << "[DEBUG identifyController] Board mode:" << boardMode;
    ...
    if (controller->boardMode != RHDUSBInterfaceBoard) {
        qDebug() << "[DEBUG identifyController] Updating wire outs...";
        dev->UpdateWireOuts();
        qDebug() << "[DEBUG identifyController] Getting number of SPI ports...";
        ...
        qDebug() << "[DEBUG identifyController] Number of SPI ports:" << controller->numSPIPorts;
        qDebug() << "[DEBUG identifyController] Expander connected:" << controller->expConnected;
    }
    qDebug() << "[DEBUG identifyController] Device identification completed successfully";
}
...
{
    qDebug() << "[DEBUG uploadFpgaBitfile] Starting FPGA configuration with file:" << filename;
    okCFrontPanel::ErrorCode errorCode = dev->ConfigureFPGA(filename.toStdString());
    qDebug() << "[DEBUG uploadFpgaBitfile] ConfigureFPGA returned error code:" << (int)errorCode;

    switch (errorCode) {
    case okCFrontPanel::NoError:
        qDebug() << "[DEBUG uploadFpgaBitfile] FPGA configuration successful";
        break;
    case okCFrontPanel::DeviceNotOpen:
        qDebug() << "[ERROR uploadFpgaBitfile] Device not open";
        QMessageBox::critical(parent, "FPGA configuration failed", "Device not open.");
        return false;
    case okCFrontPanel::FileError:
        qDebug() << "[ERROR uploadFpgaBitfile] Cannot find configuration file";
        QMessageBox::critical(parent, "FPGA configuration failed", "Cannot find configuration file.");
        return false;
    case okCFrontPanel::InvalidBitstream:
        qDebug() << "[ERROR uploadFpgaBitfile] Bitstream is not properly formatted";
        QMessageBox::critical(parent, "FPGA configuration failed", "Bitstream is not properly formatted.");
        return false;
    case okCFrontPanel::DoneNotHigh:
        qDebug() << "[ERROR uploadFpgaBitfile] FPGA DONE signal did not assert after configuration";
    ...
    case okCFrontPanel::TransferError:
        qDebug() << "[ERROR uploadFpgaBitfile] USB error occurred during download";
    ...
    case okCFrontPanel::CommunicationError:
        qDebug() << "[ERROR uploadFpgaBitfile] Communication error with firmware";
    ...
    case okCFrontPanel::UnsupportedFeature:
        qDebug() << "[ERROR uploadFpgaBitfile] Unsupported feature";
        QMessageBox::critical(parent, "FPGA configuration failed", "Unsupported feature.");
        return false;
    default:
        qDebug() << "[ERROR uploadFpgaBitfile] Unknown error, error code:" << (int)errorCode;
...
BoardSelectDialog::BoardSelectDialog(QWidget *parent) :
    ...
    playbackPorts(255)
{
    qDebug() << "[DEBUG BoardSelectDialog] === Constructor Entry ===\n";
    qDebug() << "[DEBUG BoardSelectDialog] Constructor started";
    
    try {
        qDebug() << "[DEBUG BoardSelectDialog] Setting up application info...";
    // Information used by QSettings to save basic settings across sessions.
    QCoreApplication::setOrganizationName(OrganizationName);
    QCoreApplication::setOrganizationDomain(OrganizationDomain);
    QCoreApplication::setApplicationName(ApplicationName);
    qDebug() << "[DEBUG BoardSelectDialog] Application info configured successfully";
    ...
    qDebug() << "[DEBUG BoardSelectDialog] Loading OpenCL settings...";
    QSettings settings;
    useOpenCL = settings.value("rhxUseOpenCL", true).toBool();
    qDebug() << "[DEBUG BoardSelectDialog] OpenCL setting loaded:" << useOpenCL;

    // Initialize Board Identifier.
    qDebug() << "[DEBUG BoardSelectDialog] About to create BoardIdentifier...";
    boardIdentifier = new BoardIdentifier(this);
    qDebug() << "[DEBUG BoardSelectDialog] BoardIdentifier created successfully";

    // Determine how many and what type of controllers are connected to this machine.
    qDebug() << "[DEBUG BoardSelectDialog] About to get connected controllers info...";
    controllersInfo = boardIdentifier->getConnectedControllersInfo();
    qDebug() << "[DEBUG BoardSelectDialog] Found" << controllersInfo.size() << "controllers";
    
    // Log controller details
    for (int i = 0; i < controllersInfo.size(); i++) {
        qDebug() << "[DEBUG BoardSelectDialog] Controller" << i << ":"
                 << "Serial:" << controllersInfo[i]->serialNumber
                 << "USB Version:" << controllersInfo[i]->usbVersion
                 << "Board Mode:" << controllersInfo[i]->boardMode;
    }

    // Create a table containing information about connected controllers.
    qDebug() << "[DEBUG BoardSelectDialog] Creating board table with" << controllersInfo.size() << "rows";
    boardTable = new QTableWidget(controllersInfo.size(), 3, this);
    qDebug() << "[DEBUG BoardSelectDialog] Board table created, populating...";
    populateTable();
    qDebug() << "[DEBUG BoardSelectDialog] Table populated successfully";

    qDebug() << "[DEBUG BoardSelectDialog] Creating UI elements...";
    ...
    qDebug() << "[DEBUG BoardSelectDialog] Open button created";
    ...
    qDebug() << "[DEBUG BoardSelectDialog] Playback button created";
    ...
    qDebug() << "[DEBUG BoardSelectDialog] Advanced button created";

    defaultSampleRateCheckBox = new QCheckBox(this);
    defaultSettingsFileCheckBox = new QCheckBox(this);
    qDebug() << "[DEBUG BoardSelectDialog] Checkboxes created";

    qDebug() << "[DEBUG BoardSelectDialog] Creating layouts...";
    ...
    qDebug() << "[DEBUG BoardSelectDialog] Layouts created";

    setWindowTitle("Select Intan Controller");
    setLayout(mainLayout);
    resize(minimumSize());
    qDebug() << "[DEBUG BoardSelectDialog] Window properties set";

    qDebug() << "[DEBUG BoardSelectDialog] Creating splash screen...";
    ...
    qDebug() << "[DEBUG BoardSelectDialog] Splash screen created";

    qDebug() << "[DEBUG BoardSelectDialog] Checking controller validity...";
    if (!validControllersPresent(controllersInfo)) {
        qDebug() << "[DEBUG BoardSelectDialog] No valid controllers present, showing demo dialog";
        showDemoMessageBox();
    } else {
        qDebug() << "[DEBUG BoardSelectDialog] Valid controllers found, showing dialog";
        show();
    }

    qDebug() << "[DEBUG BoardSelectDialog] Highlighting first valid row...";
    ...
    for (int row = 0; row < boardTable->rowCount(); row++) {
        ...
        if (thisText == RHDBoardString || thisText == RHS128chString ||
            ...
            boardTable->setFocus();
            qDebug() << "[DEBUG BoardSelectDialog] Highlighted row" << row << "with text:" << thisText;
            break;
        }
    }
    
    qDebug() << "[DEBUG BoardSelectDialog] Constructor completed successfully";
    
    } catch (const std::exception& e) {
        qDebug() << "[ERROR BoardSelectDialog] Standard exception in constructor:" << e.what();
        throw;
    } catch (...) {
        qDebug() << "[ERROR BoardSelectDialog] Unknown exception in constructor";
        throw;
    }
}
