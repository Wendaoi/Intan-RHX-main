...
{
    qDebug() << "[DEBUG TestControlPanel] === Constructor Entry ===\n";
    qDebug() << "[DEBUG TestControlPanel] Constructor started";
    
    try {
        qDebug() << "[DEBUG TestControlPanel] Checking parameters...";
        qDebug() << "[DEBUG TestControlPanel] controllerInterface:" << (void*)controllerInterface_;
        qDebug() << "[DEBUG TestControlPanel] rhxController:" << (void*)rhxController_;
        qDebug() << "[DEBUG TestControlPanel] state:" << (void*)state_;
        qDebug() << "[DEBUG TestControlPanel] parser:" << (void*)parser_;
        qDebug() << "[DEBUG TestControlPanel] multiColumnDisplay:" << (void*)multiColumnDisplay_;
        qDebug() << "[DEBUG TestControlPanel] stimParametersInterface:" << (void*)stimParametersInterface_;
        qDebug() << "[DEBUG TestControlPanel] parent:" << (void*)parent;
        
        qDebug() << "[DEBUG TestControlPanel] Controller type:" << (state_->getControllerTypeEnum() == ControllerStimRecord ? "ControllerStimRecord" : "Other");
        qDebug() << "[DEBUG TestControlPanel] Setting initial values...";
        reportPresent = false;
        setFocusPolicy(Qt::StrongFocus);
        setFocus();
        qDebug() << "[DEBUG TestControlPanel] Initial setup completed";

        qDebug() << "[DEBUG TestControlPanel] Creating port combo box...";
    ...
    portRow->addWidget(testAuxInsCheckBox);
        ...
        qDebug() << "[DEBUG TestControlPanel] Creating input wave check UI...";
        ...
        qDebug() << "[DEBUG TestControlPanel] Check input wave button created";
        ...
        qDebug() << "[DEBUG TestControlPanel] Help buttons created";
        ...
        qDebug() << "[DEBUG TestControlPanel] Input wave row layout created";
    ...
    qDebug() << "[DEBUG] TestControlPanel constructor completed successfully";
    ...
    qDebug() << "[DEBUG TestControlPanel] TestControlPanel initialization completed successfully";
    
    } catch (const std::exception& e) {
        qDebug() << "[ERROR TestControlPanel] Standard exception in constructor:" << e.what();
        throw;
    } catch (...) {
        qDebug() << "[ERROR TestControlPanel] Unknown exception in constructor";
        throw;
    }
}
...
{
    qDebug() << "[DEBUG] testChip function started";
    qDebug() << "[DEBUG] Controller type:" << (state->getControllerTypeEnum() == ControllerStimRecord ? "ControllerStimRecord" : "Other");
    ...
    if (state->getControllerTypeEnum() != ControllerStimRecord) {
        // DEBUG: 添加调试信息和边界检查 (testChip函数)
        qDebug() << "[DEBUG testChip] auxInData size:" << auxInData.size();
        if (auxInData.size() >= 3) {
            qDebug() << "[DEBUG testChip] auxInData[0] size:" << auxInData[0].size();
            qDebug() << "[DEBUG testChip] auxInData[1] size:" << auxInData[1].size();
            qDebug() << "[DEBUG testChip] auxInData[2] size:" << auxInData[2].size();
        }
        
        // 边界检查：确保auxInData至少有3个通道且每个通道有数据
        if (auxInData.size() >= 3 && auxInData[0].size() > 0 && auxInData[1].size() > 0 && auxInData[2].size() > 0) {
            for (int index = 0; index < auxInData[0].size(); index++) {
                // 额外的边界检查
                if (index >= auxInData[1].size() || index >= auxInData[2].size()) {
                    qDebug() << "[ERROR testChip] Index" << index << "out of bounds for auxInData[1] or auxInData[2]";
                    break;
                }
            ...
            // DEBUG: 边界检查失败的情况 (testChip函数)
            qDebug() << "[ERROR testChip] auxInData boundary check failed. auxInData.size():" << auxInData.size();
            if (auxInData.size() > 0) {
                qDebug() << "[ERROR testChip] auxInData[0].size():" << auxInData[0].size();
            }
            if (auxInData.size() > 1) {
                qDebug() << "[ERROR testChip] auxInData[1].size():" << auxInData[1].size();
            }
            if (auxInData.size() > 2) {
                qDebug() << "[ERROR testChip] auxInData[2].size():" << auxInData[2].size();
            }
            // 设置默认值
            auxIn1Median = auxIn2Median = auxIn3Median = 0.0;
        }
    } else {
        // DEBUG: ControllerStimRecord模式下不处理auxInData (testChip函数)
        qDebug() << "[DEBUG testChip] Skipping auxInData processing for ControllerStimRecord mode";
        auxIn1Median = auxIn2Median = auxIn3Median = 0.0;
    }
